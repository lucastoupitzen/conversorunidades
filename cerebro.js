function filtro() {
    ...
    switch(tipo.value) {
        case "distancia":
             res.innerHTML = '<p>Selecione a unidade de medida atual: </p><select id="distancia"><option value="inicio">Selecione</option><option value="centímetro">Centímetro</option><option value = "quilometro">Quilometro</option><option value="metro">Metro</option><option value="pé">Pé</option></select>'
        ...
        case "massa":
             ...
             break
        case "volume":
            res.innerHTML = '<p>Selecione a unidade de medida atual: </p><select id="volume"><option value="inicio">Selecione</option><option value="litros">Litro</option><option value = "m³">m³</option></select>'
            outp.innerHTML = '<p>Selecione a unidade de medida desejada: </p><select id="volumedesejado"><option value="inicio">Selecione</option><option value="litros">Litro</option><option value = "m³">m³</option></select>'
            bot.innerHTML = '<input type="button" id="disparo" value="Calcular" onclick="calculadora_volume()"></input>'
            break
...
function calculadora_distancia() {
    ...
    var dadosMedidas = {
        medidas: ['centímetro','quilometro','metro','pé'],
...
function calculadora_volume() {
    var inputnum = document.getElementById("quantia").value
    var inputini = document.getElementById("volume").value
    var inputfin = document.getElementById("volumedesejado").value
    var dadosMedidas = {
        medidas: ['litros',"m³"],
        quantRel: [1,1000]
    }
    ...
    var resultado = dadosMedidas.quantRel[controleEntrada] / dadosMedidas.quantRel[controleSaida]
    var output = document.getElementById("outpu")
    output.innerHTML = `${inputnum} ${inputini} são ${(inputnum*resultado)} ${inputfin}`
}